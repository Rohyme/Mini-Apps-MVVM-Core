...
import com.rohyme.core.presentation.appUtils.networkUtils.CustomConverterFactory
...
/*                                .addInterceptor {
                    ...
                    if (!PrefManager.newInstance().accessToken.isNullOrEmpty()) {
                        request = it.request().newBuilder().addHeader("token", PrefManager.newInstance().accessToken).build()
                    } else if (!LanguageManager.getUsedLanguage(appContext).isNullOrEmpty()) {
                        request = it.request().newBuilder().addHeader("lang", LanguageManager.getUsedLanguage(appContext)).build()
                    }
